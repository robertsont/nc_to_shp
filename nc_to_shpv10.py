          ...
          if firstFile:
              with open(outputfileloc, 'w+', newline='') as outputfile:
                ...
                while(len(outputData) > 0):
                    writer.writerow(outputData.popleft())
              firstFile = False
          else:
              with open(outputfileloc, 'r') as readfile:
                  with open("G:\\tempoutclim.csv", 'w+', newline='') as outputfile:
                    writer = csv.writer(outputfile)
                    tempReader = csv.reader(readfile)
                    for row in readfile:
                        temprow = outputData.popleft()[6:]
                        row = row.split(',')
                        row.append(temprow)
                        writer.writerow(row)
              shutil.move("G:\\tempoutclim.csv", outputfileloc)          
